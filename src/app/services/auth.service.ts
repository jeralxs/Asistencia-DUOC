...
export class AuthService {
  ...
  usuarioRecuperar = new BehaviorSubject<Usuario | null>(null);
  ...
  async pregunta(correo: string) {
        await this.bd.validarCorreo(correo).then(async (usuario: Usuario | undefined) => {
          if (usuario) {
            this.usuarioRecuperar.next(usuario);
            this.router.navigate(['pregunta']);
          } else {
            showToast(`El correo es incorrecto`);
